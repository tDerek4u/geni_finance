    ...
    <div class="p-4 sm:ml-64  h-[100vh] bg-gray-100">
        <div class="p-4  mt-14 ">
            <div
                ...
                <table id="roles" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        @foreach ($roles as $key => $role)
                            ...
                            <tr id="role-{{ $role->id }}"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    class="font-medium me-3 text-blue-600 dark:text-blue-500 hover:underline"><iconify-icon icon="fluent-mdl2:permissions" width="1.3em" height="1.3em"  style="color: black"></iconify-icon></a>
                                    <button onclick="editRole('{{ json_encode($role) }}')" id="openModalButton"
                                    ...
                                    <button data-id="{{ $role->id }}" onclick="deleteRole({{ $role->id }})"
                                        id="delete-company"
...
@section('script')
    <script>
        ...
        function closeModalButton() {
            ...
            document.getElementById('title').textContent = "Add Role";
        }
        ...
        $(document).ready(function() {
            ...
            $('#roleForm').on('submit', function(event) {
                ...
                var ajaxUrl = roleId ? `/admin/roles/${roleId}` : '/admin/roles';
                var ajaxType = roleId ? 'POST' : 'POST';
                if (roleId) {
                    formData.append('_method', 'PUT');
                }

               // use role resource url
                $.ajax({
                    url: ajaxUrl,
                    type: ajaxType,
                    ...
                    success: function(response) {
                        ...
                        if (response.status == true) {
                            ...
                            var newRow = `
                                <tr id="role-${newRole.id}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white"></td>
                                    <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">${newRole.name}</td>
                                    <td class="px-6 py-4">
                                        <button onclick="editRole('${JSON.stringify(newRole)}')" id="openModalButton" class="font-medium me-3 text-blue-600 dark:text-blue-500 hover:underline">
                                            <iconify-icon icon="mage:edit" width="1.5em" height="1.5em"></iconify-icon>
                                        </button>
                                        <button data-id="${newRole.id}" onclick="deleteRole(${newRole.id})" id="delete-company" class="font-medium text-red-600 dark:text-red-500 hover:underline">
                                            <iconify-icon icon="octicon:trash-24" width="1.5em" height="1.5em" style="color: #ff0000"></iconify-icon>
                                        </button>
                                    </td>
                                </tr>
                            ...
                            if (roleId) {
                                ...
                                $(`#role-${newRole.id}`).replaceWith(newRow);
                                updateRowIndex();
